<script lang="ts">
	...
	const productionWebhookUrl =
		'https://n8n.cambermast.com/webhook/2e4d3bc6-d83c-492f-8912-6e95dbc10d33';
	const developmentWebhookUrl =
		'https://n8n.cambermast.com/webhook-test/2e4d3bc6-d83c-492f-8912-6e95dbc10d33';
	...
	const defaultProgramSlug =
		devTestimonialPrefill?.selectedProgram ?? trainingPrograms[0]?.slug ?? '';
	...
	const testimonialsBySku = listTestimonials().reduce(
		(acc, testimonial) => {
	...
	async function submitForm(event: Event) {
		...
		try {
			...
			const programData =
				selectedProgram === 'other' ? undefined : getTrainingProgram(selectedProgram);
			...
			const res = await fetch(getWebhookUrl(), {
				...
				body: JSON.stringify({
					...
					customProgramTitle:
						selectedProgram === 'other' ? trimmedCustomProgram || undefined : undefined,
					programSku,
					programRoute,
...
<section class="mx-auto max-w-3xl space-y-6 py-10">
	...
	<div class="space-y-6 text-center">
		...
		>
			...
			<p class="text-base text-gray-700">
				If you found value from attending one of my workshops, webinars, or speaking events, please
				take a moment to share your experience. Ratings help others feel confident in making an
				informed choice. Thank you! -Bill
			</p>
		</div>
	</div>
	...
	>
		...
		<div>
				...
				{#each ratingOptions as option, index}
					...
					>
						...
						<svg
							...
							aria-hidden="true"
							style={!rating
								? `animation-delay: ${index * ratingPulseDelayIncrement}s; animation-duration: ${ratingPulseDurationSeconds}s;`
								: undefined}
						>
							<path
			...
			{#if rating}
				<p class="mt-2 text-sm text-gray-600">
					You chose {rating} star{rating === '1' ? '' : 's'} - {ratingOptions.find(
		...
		<div>
			...
			<div
				class="mt-3 rounded-2xl border border-dashed border-gray-200 bg-white px-4 py-3 text-sm text-gray-600 shadow-inner"
			>
		...
		<div>
			<label class="block text-sm font-medium text-gray-700" for="testimonial-name">
				<span class="required-label">Name to display</span>
				<span class="sr-only"> required</span>
			</label>
			<input
				class="mt-1 w-full rounded-md border px-3 py-1.5 outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-blue-500"
				...
				<div class="flex items-center gap-1 text-yellow-500" aria-hidden="true">
					{#each Array(Number(rating) || 0) as _, index}
						<svg class="h-4 w-4 fill-current" viewBox="0 0 20 20">
							<polygon
								points="10 1.5 12.9 7.26 19.2 7.97 14.3 12.28 15.6 18.48 10 15.4 4.4 18.48 5.7 12.28 0.8 7.97 7.1 7.26"
							/>
						</svg>
					{/each}
					{#if !rating}
