<script lang="ts">
	...
	const partnersBySlug = new Map(listPartners().map((partner) => [partner.slug, partner]));
	...
	$: partnerPages = Array.from(
		new Map(
			campaigns.map((campaign) => [
				campaign.partner ?? 'cambermast',
				{
...
<section class="mb-10">
	<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
		<div class="flex flex-wrap items-start justify-between gap-3">
			<div>
				<h2 class="text-xl font-semibold text-gray-900">Partner pages</h2>
				<p class="mt-1 max-w-3xl text-sm text-gray-600">
					Direct links to each partner’s campaign asset page so you can share them immediately.
				</p>
			</div>
			<a
				...
				{#each partnerPages as partner}
					<article class="rounded-xl border border-gray-200 bg-gray-50 p-4">
							...
							<div class="flex min-w-0 items-start gap-3">
								{#if partner.logo}
									<img
										...
										class="h-10 w-10 shrink-0 rounded bg-white object-contain p-1"
										loading="lazy"
									/>
								{/if}
								<div class="min-w-0">
									<p class="text-sm font-semibold text-gray-900">{partner.label}</p>
									<p class="mt-1 text-xs text-gray-500">{partner.slug}</p>
									<p class="mt-2 text-xs text-gray-600">
										{partner.campaignCount} {partner.campaignCount === 1 ? 'campaign' : 'campaigns'}
									</p>
								</div>
							</div>
							<a
						...
						<div class="mt-3 rounded-lg border border-gray-200 bg-white px-3 py-2">
							<p class="text-[11px] font-semibold tracking-wide text-gray-500 uppercase">Share URL</p>
							<div class="mt-2 flex items-center gap-2">
								<code class="min-w-0 flex-1 truncate text-xs text-gray-800">{partner.shareUrl}</code>
								<button
									type="button"
									class={iconClass(`partner:${partner.slug}`)}
									aria-label={`Copy partner page URL for ${partner.label}`}
									on:click={() => copyToClipboard(partner.shareUrl, `partner:${partner.slug}`)}
								>
									{#if copiedKey === `partner:${partner.slug}`}
										Copied
									{:else}
										Copy URL
									{/if}
		...
		{:else}
			<p class="mt-4 text-sm text-gray-600">No partner pages yet.</p>
		{/if}
	</div>
</section>
...
{:else}
	<section class="mb-10">
		<div class="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
			<p class="font-semibold">Production shows partner pages only.</p>
			<p class="mt-1">
				Use the partner links above to open shareable asset pages. The full campaign registry stays
