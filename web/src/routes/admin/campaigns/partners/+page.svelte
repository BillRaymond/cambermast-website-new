...
<section class="mb-10">
	<div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
		<div class="flex flex-wrap items-start justify-between gap-3">
			<div>
				<h2 class="text-lg font-semibold text-gray-900">Partner pages</h2>
				<p class="mt-1 max-w-3xl text-sm text-gray-600">
					Direct links to each partner asset page so you can share the right page quickly.
				</p>
			</div>
			<span class="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-700">
				{partnerPages.length} partners
			</span>
		</div>
				...
				{#each partnerPages as partner}
					<article class="rounded-xl border border-gray-200 bg-gray-50 p-4">
						...
						<div class="mt-3 rounded-lg border border-gray-200 bg-white px-3 py-2">
							<p class="text-[11px] font-semibold tracking-wide text-gray-500 uppercase">Path</p>
							<div class="mt-2 flex items-center gap-2">
								<code class="min-w-0 flex-1 truncate text-xs text-gray-800">{partner.href}</code>
								<button
									type="button"
									...
									aria-label={`Copy partner page path for ${partner.label}`}
									on:click={() => copyToClipboard(partner.href, `partner:${partner.slug}`)}
								>
