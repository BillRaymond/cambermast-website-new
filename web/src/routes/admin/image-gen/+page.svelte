<script lang="ts">
	...
	let slug = '';
	let squarePrompt = data.defaultPrompts.square;
	let landscapePrompt = data.defaultPrompts.landscape;
	let portraitPrompt = data.defaultPrompts.portrait;
	let squareN = data.defaultN;
	let landscapeN = data.defaultN;
	let portraitN = data.defaultN;

	let templates: string[] = [];
	let selectedTemplateUrl = data.defaultTemplateUrl;
	...
	let errorMessage = '';
	let saveMessage = '';
	let saveWrites: Array<{ variant: string; absolutePath: string; publicUrl: string; version: number }> = [];
	...
	const validateCount = (n: number): number => Math.min(data.maxN, Math.max(data.minN, Math.round(n)));
...
{:else}
	<section class="mt-8 space-y-6">
		...
		<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
			...
			<div class="mt-5">
				<label class="mb-2 block text-sm font-semibold text-gray-800" for="template-upload">
		...
		<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
			...
			<label class="mt-3 block text-sm font-semibold text-gray-800" for="slug">Program/Event slug</label>
			<input
				id="slug"
				type="text"
		...
		<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
			...
			<div class="mt-3 flex flex-wrap items-center gap-3">
				<label class="text-sm" for="square-count">n</label>
				<input id="square-count" type="number" min={data.minN} max={data.maxN} bind:value={squareN} class="w-24 rounded border border-gray-300 px-2 py-1 text-sm" />
				<button
		...
		<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
			...
			<div class="mt-3 flex flex-wrap items-center gap-3">
				<label class="text-sm" for="landscape-count">n</label>
				<input id="landscape-count" type="number" min={data.minN} max={data.maxN} bind:value={landscapeN} class="w-24 rounded border border-gray-300 px-2 py-1 text-sm" />
				<button
		...
		<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
			...
			<div class="mt-3 flex flex-wrap items-center gap-3">
				<label class="text-sm" for="portrait-count">n</label>
				<input id="portrait-count" type="number" min={data.minN} max={data.maxN} bind:value={portraitN} class="w-24 rounded border border-gray-300 px-2 py-1 text-sm" />
				<button
					class="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50"
					type="button"
					disabled={generatingStage !== null || !selectedSquareCandidateId}
